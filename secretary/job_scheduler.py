...
def email_daily_todos() -> None:
    ...
    calender_tuples = {
        (row['todo_calendar_id'], row['google_apis_user_id'])
        for row in
        UserTable().table.scan(ProjectionExpression='todo_calendar_id, google_apis_user_id')['Items']
    }
    for (calendar_id, google_apis_user_id) in calender_tuples:
        for event in get_todos_to_remind_today(calendar_id, google_apis_user_id):
            logging.info(f'Sent mail to {google_apis_user_id}')
